...
impl Default for SqliteConfig {
    fn default() -> Self {
        Self {
            database_path: PathBuf::from("checkpoints.db"),
            max_connections: 10,
            enable_wal: true,
            compress: true,
            cleanup_days: 30,
...
impl SqliteCheckpointer {
    ...
    pub async fn new(database_path: impl Into<PathBuf>) -> GraphResult<Self> {
    ...
    /// Create SQLite checkpointer with configuration
    pub async fn with_config(config: SqliteConfig) -> GraphResult<Self> {
        // Create database URL
        let database_url = format!("sqlite:{}", config.database_path.display());

        // Create connection pool
        let pool = SqlitePool::connect(&database_url).await.map_err(|e| {
            checkpoint_error(format!("Failed to connect to SQLite database: {}", e))
        })?;
        ...
        if config.enable_wal {
            sqlx::query("PRAGMA journal_mode = WAL")
                .execute(&pool)
                .await
                .map_err(|e| checkpoint_error(format!("Failed to enable WAL mode: {}", e)))?;
        }
    ...
    async fn init_schema(&self) -> GraphResult<()> {
        ...
        info!("SQLite checkpointer initialized successfully");
        Ok(())
    }
    ...
    fn serialize_checkpoint<S: GraphState>(
        ...
        let json_data = serde_json::to_vec(checkpoint)?;
    ...
    async fn count_checkpoints(&self) -> GraphResult<usize> {
        let row = sqlx::query("SELECT COUNT(*) as count FROM checkpoints")
            .fetch_one(&self.pool)
            .await
            .map_err(|e| checkpoint_error(format!("Failed to count checkpoints: {}", e)))?;

        Ok(row.get::<i64, _>("count") as usize)
    }

    /// Get storage size in bytes
    async fn storage_size(&self) -> GraphResult<u64> {
        let row = sqlx::query("SELECT SUM(size_bytes) as total_size FROM checkpoints")
            .fetch_one(&self.pool)
            .await
            .map_err(|e| checkpoint_error(format!("Failed to get storage size: {}", e)))?;

        Ok(row.get::<Option<i64>, _>("total_size").unwrap_or(0) as u64)
    }
}

#[async_trait]
impl<S: GraphState> Checkpointer<S> for SqliteCheckpointer
where
    S: for<'de> Deserialize<'de> + Serialize + Send + Sync,
{
    async fn put(
        ...
        let pending_writes_json = serde_json::to_string(&Vec::<PendingWrite>::new())?;
    ...
    async fn get(
        ...
        if let Some(row) = row {
            ...
            let pending_writes_json: String = row.get("pending_writes");

            let metadata: CheckpointMetadata = serde_json::from_str(&metadata_json)?;
            let pending_writes: Vec<PendingWrite> = serde_json::from_str(&pending_writes_json)?;
    ...
    async fn list(
        ...
        let mut query = "SELECT id, checkpoint_data, metadata, pending_writes, compressed FROM checkpoints WHERE thread_id = ?".to_string();
        let mut bindings = vec![config.thread_id.clone()];

        if let Some(before_id) = before {
            query.push_str(" AND created_at < (SELECT created_at FROM checkpoints WHERE id = ?)");
            bindings.push(before_id.to_string());
        }
    ...
    async fn delete(&self, config: &CheckpointConfig, checkpoint_id: &str) -> GraphResult<bool> {
        ...
        let result = sqlx::query("DELETE FROM checkpoints WHERE id = ? AND thread_id = ?")
            .bind(checkpoint_id)
            .bind(&config.thread_id)
            .execute(&self.pool)
            .await
    ...
    async fn clear_thread(&self, thread_id: &str) -> GraphResult<usize> {
        ...
        let result = sqlx::query("DELETE FROM checkpoints WHERE thread_id = ?")
            .bind(thread_id)
            .execute(&self.pool)
            .await
            .map_err(|e| {
    ...
    async fn stats(&self) -> GraphResult<CheckpointerStats> {
        ...
        let storage_size_bytes =
            stats_row.get::<Option<i64>, _>("storage_size").unwrap_or(0) as u64;
        let avg_checkpoint_size_bytes =
            stats_row.get::<Option<f64>, _>("avg_size").unwrap_or(0.0) as u64;
    ...
    async fn cleanup(&self) -> GraphResult<CleanupResult> {
        ...
        let size_query = sqlx::query(
            "SELECT SUM(size_bytes) as total_size FROM checkpoints WHERE created_at < ?",
        )
        .bind(cutoff_date)
        .fetch_one(&self.pool)
        .await
        ...
        let space_freed_bytes = size_query.get::<Option<i64>, _>("total_size").unwrap_or(0) as u64;
...
impl Default for SqliteCheckpointer {
    fn default() -> Self {
        // This will panic if called - use new() or with_config() instead
        panic!("SqliteCheckpointer::default() is not supported - use SqliteCheckpointer::new() instead")
    }
}
...
mod tests {
    ...
    async fn test_sqlite_checkpointer() {
        ...
        let checkpointer = SqliteCheckpointer::new(db_path).await.unwrap();
